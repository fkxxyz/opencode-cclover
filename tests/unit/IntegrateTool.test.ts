...
`
  ...
  test("should skip soulless employee without active session", async () => {
    ...
    // 验证记忆未被修改
    const memory = await memoryManager.read("emp_dev_1")
  ...
  test("should reset soulless employee with active session", async () => {
    ...
    // 注册 session
    sessionRegistry.register("session-1", "emp_dev_1")
  ...
  test("should reset multiple soulless employees with active sessions", async () => {
    ...
    await memoryManager.write("emp_dev_2", {
      knowledge: [],
      tasks: [],
    ...
    // 注册 sessions
    sessionRegistry.register("session-1", "emp_dev_1")
    sessionRegistry.register("session-2", "emp_dev_2")
    ...
    // 验证两个员工的记忆都被重置
    const memory1 = await memoryManager.read("emp_dev_1")
    ...
    const memory2 = await memoryManager.read("emp_dev_2")
  ...
  test("should only reset soulless employees, not soulful ones", async () => {
    ...
    // 初始化员工记忆
    await memoryManager.write("emp_soulless_1", {
      knowledge: [],
      tasks: [],
      args: {},
      sessionId: "session-1",
    ...
    await memoryManager.write("emp_soulful_1", {
      knowledge: [],
      tasks: [],
      args: {},
      sessionId: "session-2",
    ...
    // 注册 sessions
    sessionRegistry.register("session-1", "emp_soulless_1")
    sessionRegistry.register("session-2", "emp_soulful_1")
    ...
    // 验证 soulless 员工被重置
    const soullessMemory = await memoryManager.read("emp_soulless_1")
    expect(soullessMemory.sessionId).toBeUndefined()
    expect(soullessMemory.sessionSnapshot).toBeUndefined()

    // 验证 soulful 员工未被修改
    const soulfulMemory = await memoryManager.read("emp_soulful_1")
