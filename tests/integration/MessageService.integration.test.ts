...
const TEST_WORKSPACE = path.join(import.meta.dir, '../fixtures/integration-workspace')
...
describe('MessageService Integration', () => {
  ...
  test('should create correct directory structure', async () => {
    ...
    // 验证目录结构
    const calculatorDir = path.join(TEST_WORKSPACE, 'employees/calculator/messages/bayecao')
    const bayecaoDir = path.join(TEST_WORKSPACE, 'employees/bayecao/messages/calculator')
    
    const calculatorDirExists = await fs.access(calculatorDir).then(() => true).catch(() => false)
    const bayecaoDirExists = await fs.access(bayecaoDir).then(() => true).catch(() => false)
  ...
  test('should maintain synchronized message files', async () => {
    ...
    // 读取双方的消息文件
    const calculatorFilePath = service.getMessageFilePath('calculator', 'bayecao')
